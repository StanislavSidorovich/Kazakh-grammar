 ...
 * Глазами это не ловится, поэтому четыре проверки ниже.
 *
 * ВАЖНО: скрипт не хранит копию данных приложения. И словарь, и список
 * отрезаемых окончаний он вытаскивает из самого index.html — иначе разошлись
 * бы, как когда-то разошлись наборы слов и VOCAB.
 *
 * Ещё две проверки требуют браузера и живут в tools/browser-checks.js:
 * ширина таблиц и прохождение заданий на 11/11.
 */
...
// Словарь VOCAB + наборы «Слова N», которые досыпает seedVocabFromWordsets.
function buildVocab() {
  ...
  // .w может нести дополнительный класс (class="w zi") — приложение берёт
  // элемент через querySelector(".w"), значит и мы должны.
  for (const m of html.matchAll(/<span class="w[^"]*">([^<]*)<\/span>/g)) {
    vocab.add(m[1].trim().toLowerCase());
  }
  return vocab;
}
...
function buildVoicing() {
  const m = html.match(/\{\s*"ғ":"қ"[^}]*\}/);
  if (!m) throw new Error('не нашёл таблицу озвончения в stripOnce');
  const map = {};
  for (const p of m[0].matchAll(/"(.)":"(.)"/g)) map[p[1]] = p[2];
  return map;
}
...
// Повторяет lookupWord из приложения: поиск в ширину по цепочкам отрезаний
// до трёх шагов, с попыткой вернуть озвончение (кітабым → кітаб → кітап).
function stripOnce(w) {
  ...
  for (const s of STRIP) {
    if (w.length > s.length + 1 && w.slice(-s.length) === s) {
      ...
      out.push(base);
      const back = VOICING[base.slice(-1)];
      if (back) out.push(base.slice(0, -1) + back);
    }
  }
  return out;
}
...
function checkStructure() {
  ...
  for (const t of topics) {
    ...
    if (!new RegExp(`_TOPICS = \\[[^\\]]*"${t}"`).test(html)) missing.push('регистра <БЛОК>_TOPICS');
...
function checkVocabCoverage() {
  ...
  const KK = /[А-Яа-яЁёӘәҒғҚқҢңӨөҰұҮүҺһІі]+/g;
  ...
  for (const zone of zones) {
    const text = zone[1]
      ...
      .replace(/<[^>]+>/g, ' ');
    for (const m of text.matchAll(KK)) {
...
console.log('  · ширину таблиц (на теле темы доступно 343px при вьюпорте 375);');
